...
//bool cmp (VPATH i,VPATH j) { return (i.score>j.score); }

void set_PATH(VPATH* path,long label,double score,VPATH* bp){
	path->label = label;
	path->score = score;
	path->backpointer = bp;
}
...
long* trace_path(VPATH* best,long length){
  long *bestpath=(long *)my_malloc(sizeof(long)*length);
  long location;
  printf("%f ",best->score);
...
long* paths2path(long** paths,long length,long n){
  long *path=(long *)my_malloc(sizeof(long)*length*n);
  long  i,location;
  for(i=0;i<n;i++){
    for(location=0;location<length;location++){
      path[i*length+location] = paths[i][location];
    }
  }
  return path;
}

long* VPATH2long(VPATH* nbest,long length,long n){
  long   *bestpath, **nbestpath,i;
  // alloc memory TODO simplify
  nbestpath = (long **)my_malloc(sizeof(long*)*n);
  bestpath = (long *)my_malloc(sizeof(long)*length*n);
  for(i=0;i<n;i++) 
    nbestpath[i]=bestpath+i*length;

  //
  for(i=0;i<n;i++){
    nbestpath[i] = trace_path(&nbest[i],length);
  }
  printf("\n");
  bestpath = paths2path(nbestpath,length,n);
  free(nbestpath);
  return bestpath;
}

/*vector<VPATH>::iterator get_pos(vector<VPATH> *current,double s){
  // binary 
  VPATH vp;
  set_PATH(&vp,0,s,NULL);
  return lower_bound(current->begin(), current->end(), vp, cmp);

}

long* NBEST(double *trans, double **emit, long length, long classes,long n,long outn){
  double *trans_ptr,s;
  long   location,ci,p,i,end = length-1;
  vector<VPATH> m[length][classes],*current,bucket;
  VPATH  *backpointer,vp,nbest[outn];

  if(length==0) {return(NULL);}

  // Do forward pass of Viterbi.
  for(ci=1;ci<=classes;ci++) {
    set_PATH(&vp,ci,emit[0][ci],NULL);  // label,score,bp
    m[0][ci-1].push_back(vp);
  }

  for(location=1;location<=end;location++) {    // 1~length-1
    for(ci=1;ci<=classes;ci++) {  // to ci
      current=&m[location][ci-1];
      current->reserve(n+1);
      for(p=1;p<=classes;p++) {       // from p
        trans_ptr=&trans[INDEX_T1(p,0,classes)];
        for(size_t ni = 0;ni<m[location-1][p-1].size();ni++){  // n best
          backpointer=&m[location-1][p-1][ni];

          s=backpointer->score+trans_ptr[ci]+emit[location][ci];
//cout << backpointer->score << " " << trans_ptr[ci] << " " << emit[location][ci] << endl;
		  set_PATH(&vp,ci,s,backpointer);
		  //current->push_back(vp);
		  //sort(current->begin(),current->end(),cmp);

          //insert 
		  vector<VPATH>::iterator pos = get_pos(current,s);
		  current->insert(pos,vp);
		  		  
		  if((long)current->size() > n){
			current->pop_back();
		  }
		}
      }
...
//cout << "s " << s << " current " << current->score << " p " << p <<endl;
  for(ci=0;ci<classes;ci++){
	for(i=0;i<n;i++){
	  bucket.push_back(m[end][ci][i]);
	}
  }
  ...
  return VPATH2long(nbest,length,outn);
}*/
...
long *viterbi_n_best(double *trans,double **emit,long length,long classes,long n);
...
void beam_add(BEAM *beam, VPATH *_new); 
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  if(sparm->n_lattice>=0){			// n_lattice wrong code
    y.labels=viterbi_n_best(trans,emit,length,classes,sparm->n_best); // wrong
    y.length=length;
    y.n = sparm->n_lattice;
  }
...
void        write_label(FILE *fp, LABEL y)
{
  ...
  for(i=0;i<y.length;i++){
    for(j=0;j<y.n;j++)
      fprintf(fp,"%ld ",y.labels[j*y.length+i]);
    fprintf(fp,"\n");
  }
}

void        write_lattice(FILE *fp, LABEL y)
{
  /* Writes label y to file handle fp. */
  int i,j;
  for(i=0;i<y.length;i++){
    for(j=0;j<y.n;j++)
      fprintf(fp,"(%ld,%ld) ", y.labels[j*y.length*2+i*2], y.labels[j*y.length*2+i*2+1] );
    fprintf(fp,"\n");
  }
}  
...
void         parse_struct_parameters_classify(STRUCT_LEARN_PARM *sparm)
{
  ...
  for(i=0;(i<sparm->custom_argc) && ((sparm->custom_argv[i])[0] == '-');i++) {
    switch ((sparm->custom_argv[i])[2]) 
      ...
      case 'b': i++; sparm->beam_width=atol(sparm->custom_argv[i]); break;
      case 'n': i++; sparm->n_best=atol(sparm->custom_argv[i]); break;
      case 'l': i++; sparm->n_lattice=atol(sparm->custom_argv[i]); break;
      default: printf("\nUnrecognized option %s!\n\n",sparm->custom_argv[i]);
	       exit(0);
      }
  }
...
/***************************************************************************/
long *viterbi_n_best(double *trans, double **emit, long length, long classes,long n){
  //return NBEST(trans,emit,length,classes,n);
  VPATH  **path,*best=NULL,*pathbuf;
  VPATH  *current,*backpointer;
  double smax = 0,s;
  double *trans_ptr;
  long   *bestpath, **nbestpath;
  long   location,ci,p,i,end = length-1;
  ...
  pathbuf=(VPATH *)my_malloc(sizeof(VPATH)*length*classes*n);
  for(i=0;i<length;i++) 
    path[i]=pathbuf+i*classes*n;
  
  
  // Do forward pass of Viterbi. 
  for(ci=1;ci<=classes;ci++) {
	set_PATH(&path[0][ci-1],ci,emit[0][ci],NULL);  // label,score,bp
  }

  for(location=1;location<=end;location++) {	// 1~length-1
    for(p=1;p<=classes;p++) {		// from p
	  backpointer=&path[location-1][p-1];
      trans_ptr=&trans[INDEX_T1(p,0,classes)];

      for(ci=1;ci<=classes;ci++) {	// to ci
		current=&path[location][ci-1];
	    s=backpointer->score+trans_ptr[ci]+emit[location][ci];
	    if((p==1) || (current->score<s)) {
          set_PATH(current,ci,s,backpointer);
	      if((location==end) && ((smax<s) || (!best))) {
	        best=current;
	        smax=current->score;
	      }
	    }
  ...
  for(i=0;i<n;i++){
    nbestpath[i] = trace_path(&path[end][i],length);
  }
  bestpath = paths2path(nbestpath,length,n);
...
void beam_add(BEAM *beam, VPATH *_new) 
{
  ...
  if(beam->num==0) {
    beam->elem[beam->num]=_new;
    beam->num++;
    beam->worst=0;
  }
  else if(beam->num<beam->beam_width) {
    for(i=0;(i<beam->num) && (beam->elem[i]->index != _new->index);i++);
    if(i==beam->num) {
      beam->elem[beam->num]=_new;
      beam->num++;
    }
  ...
  else if(_new->score > beam->elem[beam->worst]->score) {
    for(i=0;(i<beam->num) && (beam->elem[i]->index != _new->index);i++);
    if(i==beam->num) {
      beam->elem[beam->worst]=_new;
    }
    worst_score=beam->elem[beam->worst]->score;
    for(i=0;i<beam->num;i++)
      if(worst_score>beam->elem[i]->score) {
